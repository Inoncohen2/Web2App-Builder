...
import { LoaderCircle, Download, RefreshCw, Settings2, FileCode, X } from 'lucide-react';
...
// --- Sub Component: Individual Build Card ---
interface BuildCardProps {
  appId: string;
  type: 'apk' | 'aab' | 'source' | 'ios_ipa' | 'ios_source';
  label: string;
  subLabel: string;
...
const BuildCard: React.FC<BuildCardProps> = ({ 
  appId, type, label, subLabel, icon, onStartBuild, onSavePackageName, packageName, canConfigure = false 
}) => {
  const { build, loading } = useBuildStatus(appId, type);
    ...
    `}>
       <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex flex-col items-end gap-1">
             {!isBuilding && !isReady && (
               <Button onClick={() => onStartBuild(type)} size="sm" className="h-9 px-5 bg-black text-white hover:bg-gray-800 font-bold shadow-sm">
                 Build
               </Button>
             )}
       ...
       {/* Configuration (Package Name) - Only for APK/AAB usually */}
       {canConfigure && !isBuilding && (
